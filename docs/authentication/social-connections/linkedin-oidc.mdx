...
<Callout type="info">
  If the modal or page is not still open, go to the Clerk Dashboard and go to **User & Authentication > [Social Connections](https://dashboard.clerk.com/last-active?path=user-authentication/social-connections)**. Click on the settings cog icon next to the LinkedIn option. You can now paste the **Client ID** and **Client Secret** into their respective fields.
</Callout>
