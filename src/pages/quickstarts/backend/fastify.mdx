...
Two environment variables for the Clerk SDK can be set. Below is an example of your .env file. To get the respective keys, go to the [API Keys](https://dashboard.clerk.dev/last-active?path=api-keys) page in the Clerk dashboard.
...
In this example we are going to use `dontenv` to load the environment variables. You can use any other library or method if you so wish.
...
The Clerk plugin can be registered globally or for specific routes. In this example, we will register the plugin globally.
...
If you want to use Clerk for specific routes only, you can register the plugin for specific routes. In the example below we will split the routes into two groups, one for public routes and one for private routes.
