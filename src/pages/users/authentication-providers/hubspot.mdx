...
Adding social connection with HubSpot to your app with Clerk is simple - you only need to set the **Client ID**, **Client Secret** and **Authorized redirect URI** in your instance settings.
...
> The purpose of this guide is to help you setup a HubSpot developer account  and a HubSpot OAuth app - if you're looking for step-by-step  instructions using Clerk to add social connection (OAuth) to your  application, follow the [Social connection (OAuth) guide](https://clerk.com/docs/authentication/social-connections-oauth).
...
![Configuring a HubSpot app](./hubspot/02d8bf249b4a6debe85e66ea53b5f88817acdba0-1489x995.png)

Go to the [Social Connections page](https://dashboard.clerk.dev/last-active?path=authentication/social) and enable HubSpot. In the modal that opened, ensure **Use custom credentials** is enabled and paste the values you obtained during the previous step.
...
Don't forget to click **Apply** in the Clerk dashboard. Social connection with HubSpot is now configured.
